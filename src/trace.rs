...
pub fn build_gps_trace_from_gpx(gpx:Gpx) -> Result<TraceTable<BaseElement>, Box<dyn std::error::Error>>  {
...
pub fn build_gps_trace_from_points(points: Vec<GpsPoint>) -> Result<TraceTable<BaseElement>, Box<dyn std::error::Error>>  {
    let mut track_points = points
        .into_iter()
        .map(|point| (point.lon, point.lat, point.time))
        .collect::<Vec<_>>();
    ...
    trace.fill(
        ...
        |step, state: &mut [BaseElement ] | {
            ...
            if step + 1 >= track_points.len() {
                // Если это последний шаг, не пытаемся брать следующий элемент
                return;
            }
...
//             writeln!(
//                 file,
//                 "Step {}: Latitude: {:.7}, Longitude: {:.7}, Distance to trail: {:.2} м, time: {} сек, Time difference: {} сек",
//                 i + 1, lat, lon, distance, time, time_diff
//             )?;
//         } else {
//             writeln!(
